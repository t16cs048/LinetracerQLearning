...
public class Model extends Thread implements ChangeListener {
    ...
    public int getColor(int lightNo) {
        //  System.out.println("lightNo : " + lightNo);
        ...
        switch (lightNo) {
            case Robot.LIGHT_A:
                pos = new Point2D.Double(+10, -20);
                break;
            case Robot.LIGHT_B:
                pos = new Point2D.Double(0, -20);
                break;
            case Robot.LIGHT_C:
                pos = new Point2D.Double(-10, -20);
                break;
        ...
        // 指定位置の色を取得
        if (lightNo == 2 || lightNo == 0)
            return getColorByLight(x, y);
        else
            return getColorByColor(x, y);
    }
     ...
     * @return 色
     */
    private int getColorByColor(int x, int y) {
        // 範囲外の場合は白とする
        if (x < 0 || x >= mapWidth ||
                y < 0 || y >= mapHeight)
            return Robot.WHITE;
        ...
        if (r >= 128 && g >= 128 && b >= 128) rgb |= 0xffffff;

        else if (r >= 180) rgb = 0xff0000;
        else if (g >= 180) rgb = 0x00ff00;
        else if (b >= 180) rgb = 0x0000ff;
        //System.out.println("r  = "+(argb & 0x00ff0000) + "|" + r+"  g  ="+(argb & 0x0000ff00) + "|" + g+"  b = "+(argb & 0x000000ff) + "|" + b);
        // 色判定
        switch (rgb) {
            case 0x000000:
...
//			System.out.println("----BLUE---");
    ...
    private int getColorByLight(int x, int y) {
        // 範囲外の場合は白とする
        if (x < 0 || x >= mapWidth ||
                y < 0 || y >= mapHeight)
            return Robot.WHITE;
    ...
    public boolean isOnLine() {
        ...
        for (int dx = -30; dx != +30; dx += 5) {
            ...
            // 色が黒か青ならライン上
            if ((getColorByColor(x, y) == Robot.BLACK) || (getColorByColor(x, y) == Robot.BLUE))
                return true;
        }
